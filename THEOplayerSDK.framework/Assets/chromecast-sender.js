...
(function (chrome) {
    ...
    (function (cast) {
        ...
        function requestSession(givenSuccessCallback, errorCallback, sessionRequest) {
            ...
            try {
                webkit.messageHandlers.sessionBridgeRequestSession.postMessage(successCallbackId);
            }
            catch (err) {
                console.log('Error while trying sessionBridgeRequestSession. The native context does not exist.', err);
            }
        }
        ...
        var Native = /** @class */ (function () {
            ...
            Native.prototype.createMedia = function (contentId, contentType, duration, customData, currentTime) {
                if (currentTime === void 0) { currentTime = 0; }
                this._currentMedia = new media.Media(contentId, contentType, duration, customData, currentTime);
                this._currentSession.callMediaListeners(this._currentMedia);
            };
            Native.prototype.unloadMedia = function () {
                if (this._currentMedia) {
                    this._currentMedia.unload();
                    this._currentMedia = undefined;
                }
            };
            Native.prototype.unloadSession = function () {
                if (this._currentSession) {
                    this._currentSession.unload();
                    this._currentSession = new Session(new Receiver(''));
                }
            };
            Native.prototype.reset = function () {
                this.unloadMedia();
                this.unloadSession();
            };
            return Native;
        ...
        var Session = /** @class */ (function () {
            ...
            Session.prototype.removeMediaListener = function (listener) {
                arrayRemove(this._mediaListeners, listener);
            };
            Session.prototype.addMessageListener = function (nameSpace, listener) {
                try {
                    if (Array.isArray(this._messageListeners[nameSpace])) {
                        this._messageListeners[nameSpace].push(listener);
                    }
                    else {
                        this._messageListeners[nameSpace] = [listener];
                    }
                    var param = {};
                    param.nameSpace = nameSpace;
                    param.callbackId = chromecastListenerCallbackHandler.register(listener);
            ...
            Session.prototype.loadMedia = function (loadRequest, givenSuccessCallback, errorCallback) {
                ...
                var actualSuccessCallback = function (duration) {
                    ...
                    native.createMedia(givenLoadRequest.media.contentId, givenLoadRequest.media.contentType, duration, givenLoadRequest.media.customData);
            ...
            Session.prototype.callMediaListeners = function (media) {
                for (var _i = 0, _a = this._mediaListeners; _i < _a.length; _i++) {
                    var listener = _a[_i];
                    listener(media);
                }
            };
            ...
            Session.prototype.leave = function (successCallback, errorCallback) {
                try {
                    var param = {};
                    param.callbackId = chromecastCallbackHandler.register(successCallback, errorCallback);
                    webkit.messageHandlers.sessionBridgeLeave.postMessage(JSON.stringify(param));
                }
                catch (err) {
                    console.log('Error while trying sessionBridgeLeave. The native context does not exist.', err);
                }
            };
            Session.prototype.removeMessageListener = function (nameSpace, listener) {
                ...
                try {
                    if (Array.isArray(this._messageListeners[nameSpace])) {
                        arrayRemove(this._messageListeners[nameSpace], listener);
                    }
                    ...
                    webkit.messageHandlers.sessionBridgeRemoveMessageListener.postMessage(JSON.stringify(param));
                    chromecastListenerCallbackHandler.unRegister(listener);
                }
                catch (err) {
                    console.log('Error while trying sessionBridgeRemoveMessageListener. The native context does not exist.', err);
                }
            };
            Session.prototype.removeUpdateListener = function (listener) {
                ...
                try {
                    ...
                    var callbackId = chromecastListenerCallbackHandler.lookup(listener);
                    webkit.messageHandlers.sessionBridgeRemoveUpdateListener.postMessage(callbackId);
                    chromecastListenerCallbackHandler.unRegister(listener);
                }
                catch (err) {
                    console.log('Error while trying sessionBridgeRemoveUpdateListener. The native context does not exist.', err);
                }
            };
            Session.prototype.unload = function () {
                var _this = this;
                this._mediaListeners.forEach(function (listener) {
                    _this.removeMediaListener(listener);
                });
                this._updateListeners.forEach(function (listener) {
                    _this.removeUpdateListener(listener);
                });
                this._messageListeners.forEach(function (value) {
                    _this.removeMessageListener(value.namespace, value.listener);
                });
            };
            return Session;
        }());
        ...
        (function (media_1) {
            ...
            var Media = /** @class */ (function () {
                function Media(contentId, contentType, duration, customData, currentTime) {
                    if (currentTime === void 0) { currentTime = 0; }
                    this._currentTime = 0;
                    this._cachedDate = performance.now();
                    this._supportedMediaCommands = [];
                    this.updateListeners = [];
                    this._media = new MediaInfo(contentId, contentType, duration);
                    this._media.customData = customData;
                ...
                Media.prototype.updateMediaStatus = function (parsedData) {
                    ...
                    if (parsedData.idleReason) {
                        if (parsedData.idleReason === 'null') {
                            this._idleReason = undefined;
                        }
                        else {
                ...
                Media.prototype.getEstimatedTime = function () {
                    ...
                    return Math.min(Math.max(0, unclampedEstimatedTime), upperBound);
                };
                ...
                Media.prototype.seek = function (seekRequest, successCallback, errorCallback) {
                    try {
                        ...
                        param.resumeState = seekRequest.resumeState;
                        param.callbackId = chromecastCallbackHandler.register(successCallback, errorCallback);
